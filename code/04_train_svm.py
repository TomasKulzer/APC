...
Usage:
    python code/04_train_svm.py --train ../features/combined/train.joblib --out ../features/model_svm.joblib

This imports the `train_svm` function from `code/model_training/training.py`.
"""
import os
import sys
import argparse
...
from model_training.training import train_svm, _load_data


def main(train_path: str, out_path: str, search: str = 'grid', cv: int = 5, n_jobs: int = -1):
    X_train, y_train = _load_data(train_path)
...
if __name__ == '__main__':
    ...
    p.add_argument('--train', default='../features/combined/train.joblib')
