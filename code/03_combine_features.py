...
def main():
    feat_dir = os.path.join('..', 'features')
    ...
    for name, pattern in types:
        for s in splits:
            ...
            if os.path.exists(path):
                ...
                if labels is None and lbls is not None:
                    labels = lbls
            else:
    ...
    joblib.dump({'features': X_train, 'labels': labels}, os.path.join(combined_out, 'combined_train_raw.joblib'))
    joblib.dump({'features': X_val, 'labels': labels}, os.path.join(combined_out, 'combined_val_raw.joblib'))
    joblib.dump({'features': X_test, 'labels': labels}, os.path.join(combined_out, 'combined_test_raw.joblib'))
